...
@Component({
  ...
  imports: [IonHeader, IonToolbar, IonTitle, IonContent, IonCard, IonGrid, IonCardTitle, IonCol, IonRow, IonItem, IonItem,IonSelect,IonSelectOption,FormsModule, DatePipe]
})
export class Tab2Page implements OnInit{
  ...
  protected totals = signal<MealValues>(new MealValues());
  ...
  protected readonly years : Decode[] = [ {'code' :'0', 'val': '2022'},
                                          {'code' :'1', 'val': '2023'},
                                          {'code' :'2', 'val': '2024'},
                                          {'code' :'0', 'val': '2025'},
                                          {'code' :'0', 'val': '2026'},];
  ...
  ngOnInit(): void {
    this.totals().calorieDef=1000;
    this.totals().calorieIntake=2500;
    this.totals().calorieBurnt=3500;
    this.totals().carbsIntake=250;
    this.totals().proteinIntake=200;
    this.totals().fatIntake=50;

    this.krt().showing = '1';
    this.krt().showing = '1';

    this.getTimeOptions();
    this.krt().time = this.getCurMonth();
  }
  ...
  private async getData(){
     if(this.krt().showing == "1"){
      let list: DailyValues[] = [];
      let date = new Date();
      date.setFullYear(+this.krt().time);    
      for(let a=1;a<=365;a++){
        date.setDate(a);        
      ...
      let list: DailyValues[] = [];
      let date = new Date();
      let monthLastDate = new Date(date.getFullYear(), +this.krt().time, 0).getDate();
      date.setMonth(+this.krt().time-1);    
      ...
      let list: DailyValues[] = [];
      let date = new Date();
      let curDate = date.getDay()  === 0 ? 7 : date.getDay();
      let diff = 7*+this.krt().time;
      date.setDate(date.getDate()+diff);
      for(let a=1;a<=7;a++){
        date.setDate(date.getDate()-curDate+a);        
        let key = this.formatDateKey(date);
        let item = await this.getDataFromLocalStorage(key);
        if(item != null) list.push(item);
      }
